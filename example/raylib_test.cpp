   ...
   public:
    ...
    void updateData() {
        ...
        for (int i = 0; i < enemiesCount; i++) {
            ...
            if (((enemies[i].position.x + 10 / 2) > GetScreenWidth()) || ((enemies[i].position.x + 10 / 2) < 0))
                enemies[i].speed.x *= -1;
            if (((enemies[i].position.y + 10 / 2) > GetScreenHeight()) || ((enemies[i].position.y + 10 / 2) < 0))
                enemies[i].speed.y *= -1;
        }
    }
    ...
    void updateKey() {
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
            for (int i = 0; i < 100; i++) {
                if (enemiesCount < MAX_ENEMIES) {
                    ...
                    enemies[enemiesCount].speed.x = (float)GetRandomValue(-250, 250) / 60.0f;
                    enemies[enemiesCount].speed.y = (float)GetRandomValue(-250, 250) / 60.0f;
        ...
        if (IsKeyDown(KEY_RIGHT) && playerPosition.x < screenWidth - 40) {
            playerPosition.x += 6.0f;
        }
        if (IsKeyDown(KEY_LEFT) && playerPosition.x > 0) {
            playerPosition.x -= 6.0f;
        }
        if (IsKeyDown(KEY_DOWN) && playerPosition.y < screenHeight - 40) {
            playerPosition.y += 6.0f;
        } else if (IsKeyDown(KEY_UP) && playerPosition.y > 0) {
            playerPosition.y -= 6.0f;
        }
    }
    ...
    void updateVisual() {
        ...
        DrawRectangle(static_cast<int>(playerPosition.x), static_cast<int>(playerPosition.y), 40, 40, RED);
        _camera.EndMode();
