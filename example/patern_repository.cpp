...
class Test : public benlib::patern::Repository<Test> {
   ...
   public:
    Test() : benlib::patern::Repository<Test>() { _id = benlib::patern::Repository<Test>::data().size(); }
    ~Test() {}
...
auto main() -> int {
    ...
    std::cout << "There are " << benlib::patern::Repository<Test>::data().size() << " Test object(s) in the repository." << std::endl;
    std::cout << "Remove test4" << std::endl;
    test4.reset();

    std::cout << "There are " << benlib::patern::Repository<Test>::data().size() << " Test object(s) in the repository." << std::endl;

    for (auto& test : benlib::patern::Repository<Test>::data()) {
        test->print();
    }
