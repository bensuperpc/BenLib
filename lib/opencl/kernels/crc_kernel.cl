...
__kernel void CRC32_halfbyte(__global const uchar *data, ulong length, uint previousCrc32, __global uint *resultCrc32)
{
	...
	__private const uint Crc32Lookup16[] = {0x00000000, 0x1DB71064, 0x3B6E20C8, 0x26D930AC, 0x76DC4190, 0x6B6B51F4, 0x4DB26158, 0x5005713C, 0xEDB88320,
        0xF00F9344, 0xD6D6A3E8, 0xCB61B38C, 0x9B64C2B0, 0x86D3D2D4, 0xA00AE278, 0xBDBDF21C};

	while (length-- != 0) {
        crc = Crc32Lookup16[(crc ^ *data) & 0x0F] ^ (crc >> 4);
        crc = Crc32Lookup16[(crc ^ (*data >> 4)) & 0x0F] ^ (crc >> 4);
        data++;
    }
	*resultCrc32 = ~crc;
}
...
__constant const uint Crc32Lookup[8][256] =
{{
...
{
	__private uint crc = ~previousCrc32;
	__global const uint *current = (__global const uint *)data;
	while (length >= 8) {
	...
	__global const uchar *currentChar = current;
	while (length-- != 0) {
...
{
	__private uint crc = ~previousCrc32;
	__private uint one = 0;
	__private uint two = 0;
	__global const uint *current = (__global const uint *)data;
...
__kernel void CRC32_16bytes(__global const uchar *data, ulong length, uint previousCrc32, __global uint *resultCrc32)
{
	...
	__private uint one = 0;
	__private uint two = 0;
	__private uint three = 0;
	__private uint four = 0;
	__global const uint *current = (__global const uint *)data;
